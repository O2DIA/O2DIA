...
image2DO2DIA <- function (x, select = NULL, which = select, subset = NULL, ...) {
...
# the ellipsis
   ...
   if ("legend" %in% nmdots){
    ...
    Dotmain$ylab <- expanddots(ldots$ylab, "m", nv)
    if ("clab" %in% nmdots)
      Clab <- rep(ldots$clab, length.out = nv)
    else{
      Un <- Dec[match(Which, Dec$names),"units"]
      Clab <- .O2DIA$labels[match(Un, .O2DIA$labels$Units),"Labels"]
    }    
    Dotmain$clab <- Clab
    ...
    for (ip in 1:nv) {
        ...
        List <- alist(z = z, x = times, y = grid, colkey = colkey, col = col)

        do.call("image2D", c(List, dotmain))
    }
}

image2D.O2DIAdyn <- function (z, which, ylim = NULL, ..., 
    colkey = list(cex.clab = 0.8, line.clab = 0.5, cex.axis = 0.8)) {
    if (is.null(ylim))
      ylim <- rev(range(O2DIAgrid(z)$x.int))
    if (missing(which)) which <- NULL
    image2DO2DIA(x = z, which = which, ylim = ylim, colkey = colkey, ...)
  }
...
setOldClass("O2DIAdyn")
setMethod("image2D", signature("O2DIAdyn"), image2D.O2DIAdyn)

matplot.1D.O2dia <- function (z, which, ylim = NULL, ...) {

  if (is.null(ylim))
    ylim <- rev(range(O2DIAgrid(z)$x.int))
  
  if (! inherits(z, "deSolve"))
    stop ("cannot make an image from output that is not dynamic")
  ldots <- list(...)
  nmdots <- names(ldots)
  
  Dec <- rbind(O2DIAsvar(), O2DIA1D())
  ...
  if ("xlab" %in% nmdots)
    ...
    Un <- Dec[match(Which, Dec$names),"units"]
    Xlab <- .O2DIA$labels[match(Un, .O2DIA$labels$Units),"Labels"]
  }    
  ldots$xlab <- NULL
...
matplot1D.O2DIAdyn <- function (z, which, ylim = NULL, 
  ...
  if (is.null(ylim))
   ...
   if (missing(which)) which <- NULL
   matplot.1D.O2dia(z = z, which = which, ylim = ylim, 
...
if (inherits (z, "O2DIAdyn"))
  matplot1D.O2DIAdyn(z,...)
else
...
setOldClass("O2DIAdyn")
setMethod("matplot1D", signature("O2DIAdyn"), matplot1D.O2DIAdyn)

# ==============================================================================

plot.O2dia <- function (x, ..., which, ylim = NULL) {

  if (is.null(ylim))
    ylim <- rev(range(O2DIAgrid(x)$x.int))
  
  if (! inherits(x,  "steady1D")) 
    stop ("cannot make a 1D plot from output that is not steady-state")
  ldots <- list(...)
  nmdots <- names(ldots)
...
plot.O2DIAstd <- function (x, ..., which, ylim = NULL) {
  if (is.null(ylim))
    ylim <- rev(range(O2DIAgrid(x)$x.int))
  
   if (missing(which)) which <- NULL
   plot.O2dia(x = x, ..., which = which, ylim = ylim)
  }
...
setOldClass("O2DIAstd")
setMethod("plot", signature("O2DIAstd"), plot.O2DIAstd)
